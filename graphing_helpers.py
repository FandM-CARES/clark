...
def plot_ellsworth_figs(data):
    ...
    fig, ax = plt.subplots()
    ax.scatter(
        list(em_plot["pleasantness"].values()),
        list(em_plot["anticipated_effort"].values()),
    )
    ...
    for i, txt in enumerate(emotions):
        ax.annotate(
            txt,
            (
                list(em_plot["pleasantness"].values())[i],
                list(em_plot["anticipated_effort"].values())[i],
            ),
        )
    ...
    fig, ax = plt.subplots()
    ax.scatter(
        list(em_plot["responsibility"].values()), list(em_plot["control"].values())
    )
    ...
    for i, txt in enumerate(emotions):
        ax.annotate(
            txt,
            (
                list(em_plot["responsibility"].values())[i],
                list(em_plot["control"].values())[i],
            ),
        )
    ...
    for i, txt in enumerate(emotions):
        ax.annotate(
            txt,
            (
                list(em_plot["attention"].values())[i],
                list(em_plot["certainty"].values())[i],
            ),
        )
